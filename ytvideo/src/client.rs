...
// Backup API keys from APK reference - used as fallbacks when primary key fails
const BACKUP_API_KEYS: &[&str] = &[
...
fn yt_post(endpoint: &str, body: Value) -> Result<Value, anyhow::Error> {
    ...
    let primary_result = try_yt_post_with_key(endpoint, &body_str, API_KEY);
    if primary_result.is_ok() {
        return primary_result;
    }

    // Try backup API keys as fallbacks
    for backup_key in BACKUP_API_KEYS {
        if let Ok(result) = try_yt_post_with_key(endpoint, &body_str, backup_key) {
            return Ok(result);
        }
    }
...
pub fn yt_post_continuation(
    ...
    // Try primary API key first
    let primary_result = try_yt_post_continuation_with_key(endpoint, continuation, &body_str, API_KEY);
    if primary_result.is_ok() {
        return primary_result;
    }
    ...
    // Try backup API keys as fallbacks
    for backup_key in BACKUP_API_KEYS {
        if let Ok(result) = try_yt_post_continuation_with_key(endpoint, continuation, &body_str, backup_key) {
            return Ok(result);
        }
    }
