        ...
        if (!result.title) {
          ...
          if (seoTitle && typeof seoTitle === "string") {
            ...
            if (playMatch) {
              ...
              var dashMatch = seoTitle.match(/^(.+?)\s*[-â€“]\s*(.+?)\s+on\s+Amazon/);
              if (dashMatch) {
...
function parseArtistCollectionItem(item, fallbackArtistName) {
  ...
  // secondaryText is like "Single â€¢ 2026" or "Album â€¢ 2025"
...
function handleTrackUrl(parsed) {
  ...
  // showHome returns skeletons for track pages â€” use displayCatalogTrack
...
registerExtension({
  ...
  download: function(trackID, quality, outputPath, onProgress) {
    ...
    // Call Zarz.moe media API â€” with fallback to FLAC if requested codec unavailable
    ...
    if (!apiResult && codec !== "flac") {
      L("info", "[Amazon] Codec", codec, "unavailable for ASIN:", asin, "â€” falling back to FLAC");
      codec = "flac";
