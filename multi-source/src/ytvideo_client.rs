...
use crate::ytvideo_cipher as cipher;
use crate::ytvideo_mapper as mapper;
use crate::ytvideo_parser as parser;
...
// Use credential rotator for automatic key rotation (shared with ytmusic)
...
fn yt_post(endpoint: &str, body: Value) -> Result<Value, anyhow::Error> {
    ...
    let primary_result = try_yt_post_with_key(endpoint, &body_str, get_api_key());
    ...
    for _ in 1..key_count {
        ...
        if let Ok(result) = try_yt_post_with_key(endpoint, &body_str, next_key) {
            return Ok(result);
        }
    }
...
fn try_yt_post_with_key(endpoint: &str, body_str: &str, api_key: &str) -> Result<Value, anyhow::Error> {
...
pub fn yt_post_continuation(
    ...
    // Try current API key from rotator
    let primary_result = try_yt_post_continuation_with_key(endpoint, continuation, &body_str, get_api_key());
    if primary_result.is_ok() {
        return primary_result;
    }
    ...
    for _ in 1..key_count {
        let next_key = rotate_api_key();
        if let Ok(result) = try_yt_post_continuation_with_key(endpoint, continuation, &body_str, next_key) {
            return Ok(result);
        }
    }
...
fn try_yt_post_continuation_with_key(
    endpoint: &str,
    continuation: &str,
    body_str: &str,
    api_key: &str,
...
fn player_thumbnail_to_artwork(details: &Value, video_id: &str) -> Artwork {
    ...
    if thumbs.is_empty() {
        let fallback = crate::ytvideo_parser::youtube_thumbnail_fallback(video_id);
